pipeline {
    ...
    environment {
	    ...
	    JENKINS_API_TOKEN = "1124ecdd2d677b43175c8947b29dfa7906"
    }
    stages{
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-3-95-1-71.compute-1.amazonaws.com:8080/job/CD/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
